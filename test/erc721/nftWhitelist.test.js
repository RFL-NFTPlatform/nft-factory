...
const NFTContract = artifacts.require("RFOXNFTWhitelist")
...
describe("Miss PH NFTWHitelist", function () {
  console.log("Starting NFTWHitelist...");
  ...
  beforeEach(async () => {
    ...
    nftContract = await ethers.getContractFactory("RFOXNFTWhitelist");
  ...
  it("Unwhitelist Can't mint NFTs from the presale", async function () {
    ...
    await expectRevert(missAny2.buyNFTsPresale(10, []), "Unauthorized to join the presale");
  });
  ...
  it("Whitelisted address Can mint NFTs before public start sale started and after presale started", async function () {
    ...
    await missAny2.connect(bob).buyNFTsPresale(5, []);
    expect(await missAny2.balanceOf(bob.address)).to.be.equal(5);
    await expectRevert(missAny2.buyNFTsPresale(10, []), "Unauthorized to join the presale");

    // Should revert if mint the presale exceed the limit
    await expectRevert(missAny2.connect(bob).buyNFTsPresale(1, []), "Exceed the limit");
  });
  ...
  it("Whitelisted address Can't mint NFTs if the quota for presale is full", async function () {
    ...
    await missAny2.connect(bob).buyNFTsPresale(10, bobProof);
    expect(await missAny2.balanceOf(bob.address)).to.be.equal(10);

    await expectRevert(missAny2.connect(sara).buyNFTsPresale(1, saraProof), "Exceeded Max NFTs");
  });
  ...
  it("Minting public or presale from contract should fail", async function() {
    ...
    await expectRevert(mockContractBuyer.mockBuyNFTSPresale(1), "Caller must be EOA");
  ...
  it("Whitelisted minting must use the presale price (using eth)", async function () {
    ...
    await missAnyWithETH.connect(bob).buyNFTsPresale(5, [], {value: presalePayment});
    expect(await missAnyWithETH.balanceOf(bob.address)).to.be.equal(5);
    await expectRevert(missAnyWithETH.buyNFTsPresale(10, [], {value: presalePayment}), "Unauthorized to join the presale");

    // Should revert if mint the presale exceed the limit
    await expectRevert(missAnyWithETH.connect(bob).buyNFTsPresale(1, [], {value: presalePayment}), "Exceed the limit");
  ...
  it("Mint NFTs presale should failed if sale has been finished", async function () {
    ...
    await expectRevert(missAnyWithETH.connect(bob).buyNFTsPresale(5, []), "Sale has been finished");
  });
  ...
  it("Mint NFTs presale should failed if invalid eth sent", async function () {
    ...
    await expectRevert(missAnyWithETH.connect(bob).buyNFTsPresale(5, []), "Invalid eth for purchasing");
  });
  ...
  it("Purchasing NFT presale with eth (without sale end time)", async function () {
    ...
    await missAnyWithETH.connect(bob).buyNFTsPresale(5, [], {value: wei("5", "ether")});
  });
  ...
  it("Whitelisted minting failed if payment required token but eth being sent", async function () {
    ...
    await expectRevert(missAny2.connect(bob).buyNFTsPresale(5, [], {value: "1"}), "ETH_NOT_ALLOWED");
  });
  ...
  it("Whitelisted address Can mint NFTs after public start sale started", async function () {
    ...
    await expectRevert(missAny2.connect(bob).buyNFTsPublic(10), "Sale has not been started");

    await ethers.provider.send("evm_mine", [saleStartTime+12000]);

    await missAny2.connect(bob).buyNFTsPublic(10);
    expect(await missAny2.balanceOf(bob.address)).to.be.equal(10);
  });
  ...
  it("calling presale mint if whitelist flag is not activated", async function() {
    let params = buildParams(
      ...
      saleStartTime,
      saleEndTime,
      saleStartTime+12500, // needs to consider the last evm_mine (8000)
      maxNfts,
      0,
      price,
      owner.address,
    ...
    await expectRevert(missAny2.connect(bob).buyNFTsPresale(5, []), "Sale has not been started");
    await ethers.provider.send("evm_mine", [saleStartTime+12600]);
    await missAny2.connect(bob).buyNFTsPresale(5, []);
  })
